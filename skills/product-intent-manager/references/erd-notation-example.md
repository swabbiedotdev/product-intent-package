...
This example uses SQL. For path-keyed documents, logical references without
foreign keys, embedded data, mirrors, snapshots, and aggregates, use the
[document-database conventions and worked example](document-data-models.md).
Do not transfer SQL index methods or FK enforcement to a document store.
...
flowchart TB
  ...
  RESOURCE -->|"export_job.resource_id = resource.id<br/>resource: 0..many jobs; job: 1 resource; database FK"| JOB
